...
import { getFAQGroups, type FAQCategoryId } from "@/components/screens-component/chat-screen/config";
import {
	Collapsible,
	CollapsibleContent,
	CollapsibleTrigger
...
const FAQ_EXPAND_SCROLL_DELAY_MS = 200;
...
interface FAQGroupsProps {
	...
	variant?: "drawer" | "page";
	// Landing cards can pre-expand a category; header/settings omit this.
	initialOpenCategory?: FAQCategoryId;
}
...
export function FAQGroups({ onAsk, variant = "page", initialOpenCategory }: FAQGroupsProps) {
	const { language } = useLanguage();
	const groups = getFAQGroups(language);
	const [openId, setOpenId] = useState<string | null>(() =>
		initialOpenCategory && groups.some((group) => group.id === initialOpenCategory)
			? initialOpenCategory
			: null
	);
	const categoryRefs = useRef<Partial<Record<FAQCategoryId, HTMLDivElement | null>>>({});

	const isDrawer = variant === "drawer";

	useEffect(() => {
		if (!isDrawer || !initialOpenCategory) return;
		const target = categoryRefs.current[initialOpenCategory];
		if (!target) return;

		const timer = window.setTimeout(() => {
			target.scrollIntoView({ behavior: "smooth", block: "start" });
		}, FAQ_EXPAND_SCROLL_DELAY_MS);

		return () => window.clearTimeout(timer);
	}, [isDrawer, initialOpenCategory, groups]);
			...
			{groups.map((group) => {
				...
				return (
					<div
						key={group.id}
						ref={(node) => {
							categoryRefs.current[group.id] = node;
						}}
